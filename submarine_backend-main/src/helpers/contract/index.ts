...
export const makeListTx = async (price: number, accounts: { user: PublicKey, mint: PublicKey, tokenAccount: PublicKey }) => {
  try {
    ...
    let [pool, bumpPool] = await anchor.web3.PublicKey.findProgramAddress(
      [Buffer.from(POOL_SEED), user.toBuffer()],
      new PublicKey(PROGRAM_ID)
    );

    let [poolData, bumpPoolData] = await anchor.web3.PublicKey.findProgramAddress(
      [Buffer.from(POOL_DATA_SEED), user.toBuffer(), mint.toBuffer()],
      new PublicKey(PROGRAM_ID)
    );
    ...
    instructions.push(program.instruction.list(priceHigh, priceLow, bumpPoolData, {
      accounts: {
        pool: pool,
        poolData: poolData,
        user: user,
...
export const makeUpdateListTx = async (price: number, accounts: { user: PublicKey, mint: PublicKey, tokenAccount: PublicKey }) => {
  try {
    ...
    const program = getProgram();
    let instructions: any[] = [], signers: any[] = [];
    let [poolData, bump] = await anchor.web3.PublicKey.findProgramAddress(
      [Buffer.from(POOL_DATA_SEED), user.toBuffer(), mint.toBuffer()],
      new PublicKey(PROGRAM_ID)
    );
...
export const makeCancelListTx = async (accounts: { user: PublicKey, mint: PublicKey, tokenAccount: PublicKey }) => {
  try {
    ...
    instructions.push(program.instruction.cancelList(bumpPool, {
      accounts: {
        pool: pool,
        poolData: poolData,
        user: user,
...
export const makeBuyTx = async (accounts: { buyer: PublicKey, seller: PublicKey, mint: PublicKey, tokenFrom: PublicKey, marketFee: number }) => {
  try {
    const { buyer, seller, mint, tokenFrom, marketFee } = accounts;
    const program = getProgram();
    let instructions: any[] = [], signers: any[] = [];
    let [pool, bumpPool] = await anchor.web3.PublicKey.findProgramAddress(
      [Buffer.from(POOL_SEED), seller.toBuffer()],
      new PublicKey(PROGRAM_ID)
    );

    let [poolData, bumpPoolData] = await anchor.web3.PublicKey.findProgramAddress(
      [Buffer.from(POOL_DATA_SEED), seller.toBuffer(), mint.toBuffer()],
      new PublicKey(PROGRAM_ID)
    );
    ...
    let lamportsFee = marketFee * LAMPORTS_PER_SOL;
    const feeHigh = Math.floor(lamportsFee / LAMPORTS_PER_SOL);
    const feeLow = lamportsFee % LAMPORTS_PER_SOL;

    console.log('fee', feeHigh, '_', feeLow)

    instructions.push(program.instruction.buy(0, feeHigh, feeLow, {
      accounts: {
        pool: pool,
        poolData: poolData,
        buyer: buyer,
...
export const makeAcceptBidTx = async (accounts: { seller: PublicKey, bidder: PublicKey, mint: PublicKey, tokenFrom: PublicKey, vault: Keypair, marketFee: number }) => {
  try {
    const { seller, bidder, mint, tokenFrom, vault, marketFee } = accounts;
    const program = getProgram();
    let instructions: any[] = [], signers: any[] = [];
    ...
    let [poolPda, bumpPool] = await anchor.web3.PublicKey.findProgramAddress(
      [Buffer.from(POOL_SEED), seller.toBuffer()],
      new PublicKey(PROGRAM_ID)
    );

    let [poolDataPda, bumpPoolData] = await anchor.web3.PublicKey.findProgramAddress(
      [Buffer.from(POOL_DATA_SEED), seller.toBuffer(), mint.toBuffer()],
      new PublicKey(PROGRAM_ID)
    );
    ...
    instructions.push(program.instruction.acceptBid(0, feeHigh, feeLow, {
      accounts: {
